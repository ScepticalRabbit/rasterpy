...
# ==============================================================================
"""Render polynomial distorted references using Riley rasteriser."""
...
def render_preset(preset: PolyPreset) -> tuple[np.ndarray, np.ndarray]:
    """Render a single polynomial preset using Riley."""
    ...
    camera = riley.Camera(
        ...
        distortion_model=3,  # polynomial
        distortion_poly_order=preset.order.value,
        distortion_poly_has_forward=True,
        distortion_poly_forward_u=preset.coeffs_u,
        distortion_poly_forward_v=preset.coeffs_v,
    )
...
def main() -> None:
    """Execute all Case 3 Riley renders."""
    print("=" * 70)
    print("Running Case 3: Riley Standalone Polynomial Renders")
    print("=" * 70)

    OUT_RILEY_DIR.mkdir(parents=True, exist_ok=True)

    for preset in POLY_PRESETS:
    ...
    print("\nCase 3 Riley renders completed successfully.")
